...
void AnalogCalibration::update(uint8_t ms) {
    if (_updated) {
        const uint16_t bg = RGB565(33, 35, 36);
        ...
        if (_updated == 255) {
            ...
            // start button
            gfx.drawRect(156, 260, 168, 46, darkgray);
            gfx.drawFastHLine(157, 261, 166, white);
            gfx.drawFastVLine(157, 262, 42, white);
            gfx.drawFastVLine(322, 262, 42, white);
            gfx.drawFastHLine(157, 304, 166, darkgray);
            gfx.fillRect(158, 262, 164, 42, lightgray);
            gfx.setTextColor(darkgray);
            gfx.setCursor(191, 279);
            gfx.print(F("Start Calibration"));
